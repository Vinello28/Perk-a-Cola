...
def _read_column(file_bytes: bytes, file_name: str, column_name: str) -> list[str]:
    ...
    if file_name.endswith('.csv'):
        df = pd.read_csv(io.BytesIO(file_bytes), usecols=[column_name])
    else:
        df = pd.read_excel(io.BytesIO(file_bytes), usecols=[column_name])
...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader(
        "Upload an Excel or CSV file (.xlsx, .csv)",
        type=["xlsx", "csv"],
...
if uploaded_file is None:
    ...
    if result_container.get("error"):
        ...
        mime_type = "text/csv" if is_csv else "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"

        with tempfile.NamedTemporaryFile(suffix=suffix, delete=False) as tmp:
            tmp_path = Path(tmp.name)
            write_results(descriptions, labels, tmp_path)
            file_bytes_out = tmp_path.read_bytes()

        st.download_button(
            label=f"⬇️  Scarica risultati ({suffix})",
            data=file_bytes_out,
            file_name=f"{Path(uploaded_file.name).stem}_classificata{suffix}",
            mime=mime_type,
            type="primary",
            use_container_width=True,
        )
